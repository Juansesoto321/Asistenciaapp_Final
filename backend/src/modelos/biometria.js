...
async function esAprendiz(idUsuario) {
  const r = await pool.query("SELECT 1 FROM usuario WHERE id_usuario = $1 AND rol = 'aprendiz'", [idUsuario]);
  return !!r.rows[0];
}

async function esAprendizDeInstructor(idAprendiz, idInstructor) {
  const r = await pool.query(
    `SELECT 1 FROM matricula m JOIN ficha f ON f.id_ficha = m.id_ficha
     WHERE m.id_aprendiz = $1 AND m.estado = 'activa'
       AND (f.id_instructor = $2 OR EXISTS (
         SELECT 1 FROM horario h WHERE h.id_ficha = f.id_ficha AND h.id_instructor = $2))`,
...
async function eliminarPlantilla(cliente, idAprendiz) {
  const r = await cliente.query(
...
async function revocarConsentimiento(cliente, idConsentimiento) {
  await cliente.query("UPDATE consentimiento SET revocado_en = NOW() WHERE id_consentimiento = $1", [idConsentimiento]);
}

async function notificarBorrado(cliente, idAprendiz) {
  await cliente.query(
