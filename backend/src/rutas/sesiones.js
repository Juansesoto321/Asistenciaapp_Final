...
async function generarJustificacionYNotificar(
  cliente,
  { idAsistencia, idAprendiz, numeroFicha }
) {
  const horasConf = await cliente.query(
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (
        req.usuario.rol === "instructor" &&
        h.rows[0].id_instructor !== req.usuario.id
      ) {
        return res.status(403).json({
          mensaje: "Solo el instructor titular puede iniciar esta sesión",
        });
      }
      ...
      const r = await pool.query(
        `INSERT INTO sesion_clase (id_horario, fecha)
         ...
         ON CONFLICT (id_horario, fecha)
         DO UPDATE SET estado = sesion_clase.estado
      ...
      if (sesion.estado === "cerrada") {
        return res.status(400).json({
          mensaje: "La sesión de hoy ya fue cerrada",
        });
      }
      ...
      res.json({
        ...
        mensaje:
          h.rows[0].estado_lector === "en_linea"
      ...
      res.status(500).json({
        mensaje: "Error al iniciar la sesión",
      });
    }
  }
);
...
router.get("/:id", async (req, res) => {
  ...
  const aprendices = await pool.query(
    `SELECT u.id_usuario,
     ...
     JOIN horario h
      ...
      AND h.id_ficha = m.id_ficha
     LEFT JOIN asistencia asi
       ON asi.id_sesion = s.id_sesion
      AND asi.id_aprendiz = u.id_usuario
     LEFT JOIN plantilla_biometrica pb
       ON pb.id_aprendiz = u.id_usuario
  ...
  res.json({
    ...r.rows[0],
    aprendices: aprendices.rows,
  });
});

// CU-15: registro manual / override
// Permite modificar asistencia incluso despues del cierre.
// Toda modificacion requiere motivo y queda registrada en cambio_asistencia.
router.post(
  "/:id/asistencia-manual",
  autorizar("instructor", "administrador"),
  async (req, res) => {
    ...
    try {
      ...
      if (!motivo?.trim()) {
        return res.status(400).json({
          mensaje:
            "Todo registro manual requiere una justificación (regla CU-15)",
        });
      }
      ...
      // Se permite modificar la asistencia aunque la sesión ya esté cerrada.
      // La modificación queda registrada en cambio_asistencia para mantener
      // la trazabilidad de quién realizó el cambio y por qué.
      const s = await pool.query(
        "SELECT estado FROM sesion_clase WHERE id_sesion = $1",
        [req.params.id]
      );
      ...
      const u = await pool.query(
        "SELECT nombres, apellidos FROM usuario WHERE id_usuario = $1",
        [id_aprendiz]
      );
      ...
      await auditar(
        ...
        idAsistencia,
        {
          estado,
          motivo,
          sesion_cerrada: s.rows[0].estado === "cerrada",
        }
      );
      ...
      res.status(500).json({
        mensaje: "Error en el registro manual",
      });
    } finally {
      cliente.release();
    }
...
// Cierre de sesion:
// marca ausentes + genera enlaces de justificacion (72h) + notifica
router.post(
  "/:id/cerrar",
  autorizar("instructor", "administrador"),
  async (req, res) => {
    ...
    try {
      ...
      if (s.rows[0].estado === "cerrada") {
        return res.status(400).json({
          mensaje: "La sesión ya está cerrada",
        });
      }

      if (
        req.usuario.rol === "instructor" &&
        s.rows[0].id_instructor !== req.usuario.id
      ) {
        return res.status(403).json({
          mensaje: "Solo el instructor titular puede cerrar la sesión",
        });
      }
      ...
      const ausentes = await cliente.query(
        `INSERT INTO asistencia
           (id_sesion, id_aprendiz, estado, metodo)
         ...
         WHERE m.id_ficha = $2
           ...
           AND NOT EXISTS (
             ...
             FROM asistencia a
             WHERE a.id_sesion = $1
               AND a.id_aprendiz = m.id_aprendiz
           )
      ...
      // Por cada ausente:
      // enlace de justificacion + correo + notificacion
      for (const a of ausentes.rows) {
        await generarJustificacionYNotificar(cliente, {
          idAsistencia: a.id_asistencia,
          idAprendiz: a.id_aprendiz,
          numeroFicha: s.rows[0].numero_ficha,
        });
      }
      ...
      await auditar(
        ...
        {
          ausentes: ausentes.rows.length,
        }
      );

      res.json({
        mensaje: `Sesión cerrada. ${ausentes.rows.length} aprendiz(es) marcados como ausentes y notificados`,
      });
    } catch (e) {
      await cliente.query("ROLLBACK");
